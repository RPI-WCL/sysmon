...
#if 0 /* Following code causes Segmentation Fault, so disable it for now */
 ...
 * (moved from iostat.c by Shigeru)
 * 
 * IN:
 ...
 ***************************************************************************
 */
void read_sysfs_disk(struct stats_sysfs_disk *st_disk, char *dev) 
{
	FILE *fp;
	char filename[64];
	int i;
    ...
    /* Read speed info */
    sprintf(filename, SYSFS_DISK, dev);
	...
	if (i == 11) {
		...
		st_disk->rd_merges  = rd_merges_or_rd_sec;
		st_disk->rd_sectors = rd_sec_or_wr_ios;
		st_disk->rd_ticks   = (unsigned int) rd_ticks_or_wr_sec;
		st_disk->wr_ios     = wr_ios;
		st_disk->wr_merges  = wr_merges;
		st_disk->wr_sectors = wr_sec;
		...
		st_disk->rd_ios     = rd_ios;
		st_disk->rd_sectors = rd_merges_or_rd_sec;
		st_disk->wr_ios     = rd_sec_or_wr_ios;
		st_disk->wr_sectors = rd_ticks_or_wr_sec;
	}
