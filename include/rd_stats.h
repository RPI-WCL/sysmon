...
#define SYSFS_DISK      "/sys/block/%s/stat"
...
/* Structure for sysfs block devices statistics */
struct stats_sysfs_disk {
	unsigned long rd_ios        __attribute__ ((aligned (16)));
	unsigned long rd_merges     __attribute__ ((aligned (16)));
	unsigned long rd_sectors	__attribute__ ((aligned (16)));
	unsigned long wr_ios        __attribute__ ((aligned (16)));
	unsigned long wr_merges     __attribute__ ((aligned (16)));
	unsigned long wr_sectors    __attribute__ ((aligned (16)));
	unsigned int rd_ticks       __attribute__ ((aligned (8)));
	unsigned int wr_ticks       __attribute__ ((aligned (8)));
	unsigned int ios_pgr        __attribute__ ((aligned (8)));
	unsigned int tot_ticks		__attribute__ ((aligned (8)));
	unsigned int rq_ticks		__attribute__ ((packed));
};

#define STATS_SYSFS_SIZE	(sizeof(struct stats_sysfs_disk));
...
void read_sysfs_disk
	(struct stats_sysfs_disk *, char *);
